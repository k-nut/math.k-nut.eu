...
isSinewaveboxTurned = false;

function turnbox(){
	var deg = 90;
	var margin = 125;
	if (isSinewaveboxTurned){
		deg = 0;
		margin = 0;
	}
	isSinewaveboxTurned = !isSinewaveboxTurned;
	$(".sinewavebox").css('-webkit-transform','rotate('+deg+'deg)');
	$(".sinewavebox").css('-moz-transform','rotate('+deg+'deg)');
	$(".sinewavebox").css("margin-top", margin);
}
...
function newsin(){
	...
	var cosinus = [[0,height], [distance, height]];

	sinepointer.push([0,0], [distance, height]);

	sinewave_plot.setData( 
		[{data: sinewave},
			{data: [[angle, 0], [angle, distance]],
				color: "red"
			}
	]);
	sinewave_plot.draw();
	...
	new_plotdata_unit_circle[3] = {data: cosinus, color:"red"};
